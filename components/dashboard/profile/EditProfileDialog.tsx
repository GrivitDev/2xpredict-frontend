...
import { Pencil, User2, Phone, Save } from 'lucide-react';
import { z } from 'zod';
...
export default function EditProfileDialog({ user }: Props) {
  ...
  const inputClassName = `
    h-10 rounded-lg border-border/60 bg-muted/[0.22]
    ...
    <Sheet open={open} onOpenChange={handleOpenChange}>
      <SheetTrigger asChild>
        <Button
          variant="outline"
          className="
            h-9 rounded-lg border-primary/25
            bg-primary/[0.05] px-3.5 text-s
      ...
      <SheetContent
        side="right"
        className="
          w-full border-l border-border/70
              ...
              <div className="flex-1 overflow-y-auto px-5 py-5">
                <div className="space-y-4">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-s font-semibold">
                          Full Name
                        </FormLabel>
              ...
              >
                <div className="flex justify-end gap-2">
                  <Button
                    ...
                    className="
                      h-9 rounded-lg border-border/60
                      px-4 text-s font-semibold shadow-none
                    "
                  >
                    Cancel
                  </Button>
                  ...
                  <Button
                    ...
                    disabled={updating}
                    className="
                      h-9 rounded-lg bg-primary
                      px-4 text-s font-semibold
